# Benchmark (directed|undirected|trajectories) for all test networks
import numpy as np
...
# Directed inference from trajectories
algoT = ['SCRIBE_timed', 'SCRIBE_pseudotimed', 'SCRIBE_wadd']

# List of benchmarks
benchmarks = ['Network4', 'Cycle', 'Trifurcation', 'Bifurcation',
...
grid = gs.GridSpec(5, 3, hspace=0.3, wspace=0.18,
    height_ratios=[1,1,1,1,1], width_ratios=[5.6,3.7,2.5])
...
auprTreesT = {algo: [] for algo in algoT+['Random']}
...
for n, benchmark in enumerate(benchmarks):
    ...
    auprT = {algo: [] for algo in algoT+['Random']}
    ...
    for r in range(1,N+1):
        ...
        # 3. Directed inference from trajectories
        edges = [(i,j) for i in range(G) for j in set(range(1,G))-{i}]
        y0 = np.array([inter[i,j] for (i,j) in edges])
        auprT['Random'].append(np.mean(y0))
        if benchmark[:5] != 'Trees':
            for algo in algoT:
                score = abs(np.load(path+f'{benchmark}/{algo}/score_{r}.npy'))
                y1 = np.array([score[i,j] for (i,j) in edges])
                precision, recall, thresholds = precision_recall_curve(y0, y1)
                auprT[algo].append(auc(recall,precision))
        elif benchmark != 'Trees100':
            for algo in algoT:
                s = '_scribe' if algo == 'SCRIBE_timed' else ''
                u = r if benchmark != 'Trees50' else 1
                if algo == 'SCRIBE_timed' and benchmark == 'Trees5': u = 1
                if algo == 'SCRIBE_timed' and benchmark == 'Trees50': u = 1+r%2
                if algo == 'SCRIBE_pseudotimed':
                    if benchmark in {'Trees10', 'Trees50'}: u = 2
                inter = abs(np.load(path+f'{benchmark}/True/inter_{u}{s}.npy'))
                y0 = np.array([inter[i,j] for (i,j) in edges])
                score = abs(np.load(path+f'{benchmark}/{algo}/score_{u}.npy'))
                y1 = np.array([score[i,j] for (i,j) in edges])
                precision, recall, thresholds = precision_recall_curve(y0, y1)
                auprT[algo].append(auc(recall,precision))
    ...
    if benchmark[:5] == 'Trees':
        ...
        if benchmark != 'Trees100':
            for algo in algoT+['Random']:
                auprTreesT[algo].append(np.mean(auprT[algo]))

    # Non-tree benchmarks
    name = ['FN4', 'CN5', 'FN8', 'BN8']
    if n < 4:

        # A. Snapshot-based (undirected edges)
        ax = plt.subplot(grid[n,0])
        configure(ax)
        if n == 0:
            title = 'Snapshot-based (undirected edges)'
            ax.annotate('A', xytext=(x,y), fontweight='bold', **opt)
        ...
        # B. Snapshot-based
        ax = plt.subplot(grid[n,1])
        configure(ax)
        if n == 0:
            title = 'Snapshot-based'
            ax.annotate('B', xytext=(x+0.2,y), fontweight='bold', **opt)
            ax.annotate(title, xytext=(x+14,y), **opt)
        # Draw the baseline
        ...
        if n == 0:
            ...
            ax.annotate('C', xytext=(x,y), fontweight='bold', **opt)
            ax.annotate(title, xytext=(x+14,y), **opt)
        # Draw the baseline
        b = np.mean(auprT['Random'])
        ax.plot([0,4], [b,b], color='lightgray', ls='--', lw=0.8, zorder=0)
        ax.set_xlim(0.75,3.35)
        # Draw the histograms
        for i, algo in enumerate(algoT):
            box = ax.boxplot([auprT[algo]], positions=[i+1], **opt_box)
            configure_box(box, c[algo])
        ax.set_xticklabels(['Real traj.', 'Pseudotime', 'Coupling'])
...
configure(ax)
# Draw the curves
l = ['Real traj.', 'Pseudotime', 'Coupling', 'Random']
for i, algo in enumerate(algoT+['Random']):
    ax.plot(size[:-1], auprTreesT[algo], color=c[algo][0], label=l[i], **s)
ax.legend(loc='upper right', **p)
ax.set_xticks(size[:-1])
ax.set_xlabel('No. of genes', fontsize=6, labelpad=1.8)
