...
for r in range(N):
    ...
    # True network
    file = 'Trifurcation/True/inter_{}.npy'.format(r+1)
    NETWORK[r] = abs(np.load(file))
    # HARISSA
    file = 'Trifurcation/HARISSA/score_{}.npy'.format(r+1)
    HARISSA[r] = abs(np.load(file))
    # CARDAMOM
    file = 'Trifurcation/CARDAMOM/score_{}.npy'.format(r + 1)
    CARDAMOM[r] = abs(np.load(file))
    # GENIE3
    file = 'Trifurcation/GENIE3/score_{}.npy'.format(r + 1)
    GENIE3[r] = abs(np.load(file)).T
    # SINCERITIES
    file = 'Trifurcation/SINCERITIES/score_{}.npy'.format(r + 1)
    SINCERITIES[r] = abs(np.load(file))
    # PIDC
    file = 'Trifurcation/PIDC/score_{}.npy'.format(r + 1)
    PIDC[r] = abs(np.load(file))
    # PEARSON
    file = 'Trifurcation/PEARSON/score_{}.npy'.format(r + 1)
...
# Export
fig.savefig('results_Trifurcation_comp.pdf', bbox_inches='tight')