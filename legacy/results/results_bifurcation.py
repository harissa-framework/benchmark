...
for r in range(N):
    ...
    # True network
    file = 'Bifurcation/True/inter_{}.npy'.format(r+1)
    NETWORK[r] = abs(np.load(file))
    # HARISSA
    file = 'Bifurcation/HARISSA/score_{}.npy'.format(r+1)
    HARISSA[r] = abs(np.load(file))
    # CARDAMOM
    file = 'Bifurcation/CARDAMOM/score_{}.npy'.format(r + 1)
    CARDAMOM[r] = abs(np.load(file))
    # GENIE3
    file = 'Bifurcation/GENIE3/score_{}.npy'.format(r + 1)
    GENIE3[r] = abs(np.load(file)).T
    # SINCERITIES
    file = 'Bifurcation/SINCERITIES/score_{}.npy'.format(r + 1)
    SINCERITIES[r] = abs(np.load(file))
    # PIDC
    file = 'Bifurcation/PIDC/score_{}.npy'.format(r + 1)
    PIDC[r] = abs(np.load(file))
    # PEARSON
    file = 'Bifurcation/PEARSON/score_{}.npy'.format(r + 1)
...
ax.text(0.091+w, h, 'Mean trajectories', fontsize=14,
    transform=ax.transAxes,
    horizontalalignment='left', verticalalignment='bottom')
...
# Export
fig.savefig('results_Bifurcation_comp.pdf', bbox_inches='tight')