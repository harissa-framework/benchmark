...
for r in range(N):
    ...
    # True network
    file = 'Cycle/True/inter_{}.npy'.format(r+1)
    NETWORK[r] = abs(np.load(file))
    # HARISSA
    file = 'Cycle/HARISSA/score_{}.npy'.format(r+1)
    HARISSA[r] = abs(np.load(file))
    # CARDAMOM
    file = 'Cycle/CARDAMOM/score_{}.npy'.format(r + 1)
    CARDAMOM[r] = abs(np.load(file))
    # GENIE3
    file = 'Cycle/GENIE3/score_{}.npy'.format(r + 1)
    GENIE3[r] = abs(np.load(file)).T
    # SINCERITIES
    file = 'Cycle/SINCERITIES/score_{}.npy'.format(r + 1)
    SINCERITIES[r] = abs(np.load(file))
    # PIDC
    file = 'Cycle/PIDC/score_{}.npy'.format(r + 1)
    PIDC[r] = abs(np.load(file))
    # PEARSON
    file = 'Cycle/PEARSON/score_{}.npy'.format(r + 1)
...
# Export
fig.savefig('results_Cycle_comp.pdf', bbox_inches='tight')
