...
def distance_matrix(data, verb=False):
        ...
        for g in range(1,G):
            if verb: 
                print('Computing distances for gene {}'.format(g))
            times = data[:,0]
            x = data[:,g]
            for i in range(T-1):
                x1 = x[times==t[i]]
        ...
        # Symbolize the stimulus with distance 1
        d[0,0] = scale[0] * 1
        return d

def score_matrix(d, alpha=1, l1=1e-8, verb=False):
    """
    ...
    # Possible choice by leave-one-out cross validation
    if alpha is None:
        n_alphas = 10
        valpha = np.logspace(-2,1,n_alphas)
        verror = np.zeros(n_alphas)
        vtest = np.arange(T-1)
        if verb: 
            print('Cross validation (alpha -> error):')
        ...
        model.alpha = alpha
    if verb: 
        print('Choosing alpha = {:.2e}'.format(model.alpha))
    model.fit(x,y)
    f = model.sparse_coef_.transpose()
    return f

def sincerities(data, nodiag=False, verb=False):
    """
    Compute a sparse matrix f by keeping only the most likely links,
    ...
    for j in range(x[0].size):
        x[:,j] = x[:,j]/np.sqrt(np.var(x[:,j]))
    model = EmpiricalCovariance()
    model.fit(x)
    p = model.precision_
    # Estimate score matrix
...
InferencesGenerator.register("Sincerities", Sincerities)